...
'''
@File    :   coroweb.py
@Time    :   2021/06/29 16:50:31
@Author  :   xianglun xu 
@Contact :   xianglun918@qq.com
'''

# 从实用者的角度出发aiohttp搭成的Web框架比较底层，我们还需要搭建一层更为方便的框架
# 一般来说，编写一个处理URL的函数需要这么几步：
# 1. 编写一个异步的hander_url_xxx函数
# 2. 传入的参数需要自己从request中获取
# 3. 最后，需要自己构造response的对象
# 这些重复的工作可以由框架来完成。编写的简单函数可以单独测试，而不需要模拟一个request来测试。
