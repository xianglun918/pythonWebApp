...
# 编写Api：之前我们已经定义了返回主页的函数index
# 现在让我们把Web的全部功能封装一下，这样我们可以
# 让前端和后端的代码隔离，使得后段代码易于测试，前端代码编写
# 更简单。一个API也是一个处理URL的处理函数，我们希望能直接通过一个
# @api来把函数变成JSON格式的REST API，这样，获取注册用户
# 就可以用如下一个API实现：
...
async def api_get_users(*, page='1'):
    ...
    users = await User.findAll(orderBy='created_at desc', limit=(p.offest, p.limit))
    for u in users:
        u.passwd = '******'
    return dict(page=p, users=users)

# 只需要返回一个dict,后续的response这个middleware就可以把结果序列化为JSON并返回
...
import re, time, json, logging, hashlib, base64, _asyncio
...
## 编辑日志API
@post('/api/blogs/{id}')
async def api_update_blog(id, request, *, name, summary, content):  # 注意*用于分割位置参数和关键字参数，此处没有可变参数
    check_admin(request)
    blog = await Blog.find(id)
    if not name or not name.strip():
        raise APIValueError('name', 'name cannot be empty.')
